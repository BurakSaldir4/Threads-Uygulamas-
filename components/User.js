...
const User = ({ item }) => {
  ...
  const sendFollow = async (currentUserId, selectedUserId) => {
    try {
      const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL}/follow`, {
        method: "POST",
        headers: {
  ...
  const handleUnfollow = async (targetId) => {
    try {
      const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL}/users/unfollow`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  return (
    <View>
      <View style={{
        ...
        />
        <Text style={{ fontSize: 15, fontWeight: "500", color: "black", flex: 1 }}>{item.name}</Text>
